  ...
  fetch(apiEndpoint)
    ...
    .then((data) => {
      ...
      if (usageCount && usageCount.value > 0) {
        ...
        alert(
          "Foreground image not applied because usage_count is zero or not found."
        );
      }
    })
    .catch((error) => {
...
function downloadSVG() {
  ...
  // Add CDATA section with API fetch logic and optional image composition
  const scriptFetch = `
                <![CDATA[
                    (function() {
                        const apiEndpoint = "${
                        ...
                        fetch(apiEndpoint)
                            ...
                            .then(data => {
                                ...
                                if (usageCount && usageCount.value > 0 && fgImage) {
            ...
            `;
  source = source.replace("</svg>", `<script>${scriptFetch}</script></svg>`);
